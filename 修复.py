...
def create_api_service_file():
    """创建api_service.py文件"""
    api_service_content = '''# api_service.py
import os
import json
import threading
import cv2
...
def check_dependencies():
    ...
    try:
        import flask
        import flask_cors
        print("Flask和Flask-CORS已安装")
        return True
    except ImportError as e:
        print(f"依赖未安装: {e}")
        print("请执行: pip install flask flask-cors")
        return False
...
def main():
    ...
    try:
        ...
        if check_dependencies():
            print("所有依赖已安装，API服务可以正常使用")
        else:
            print("请安装缺失的依赖")

        print("\n修复完成！")
        print("现在可以重新运行系统，API服务应该可以正常使用了")
