...
class FaceRecognitionUtils:
    ...
    def select_image(self):
        ...
        try:
            ...
            if file_path:
                # 使用稳健的图像加载方法
                image = self.load_image_safely(file_path)
                if image is None:
                    return
    ...
    def load_image_safely(self, file_path):
        ...
        try:
            ...
            # 重新打开图像，因为verify()会关闭文件
            image = Image.open(file_path)

            # 记录图像信息
            self.parent.update_log(
                f"加载图像: {os.path.basename(file_path)}, 格式: {image.format}, 模式: {image.mode}, 尺寸: {image.size}")

            return image

        except Exception as load_error:
            self.parent.update_log(f"标准图像加载失败: {str(load_error)}")
            ...
            try:
                ...
                self.parent.update_log(f"使用OpenCV成功加载图像")

                return image

            except Exception as cv_error:
                self.parent.update_log(f"OpenCV加载也失败: {str(cv_error)}")
                QMessageBox.critical(self.parent, "错误", "图像文件损坏或格式不支持")
                return None
        ...
        try:
            ...
            if file_path:
                ...
                try:
                    ...
                    self.parent.update_log(f"成功保存照片: {photo_path}")

                except Exception as save_error:
                    self.parent.update_log(f"保存照片失败: {str(save_error)}")
                    self.parent.enrollment_status.setText("保存照片失败")
                    return
